import {
  ...
  import useCollaborationList from "@/hooks/admin/useCollaborationList";
  import AddCollaborationDialog from "@/components/admin/AddCollaborationDialog";
  import UpdateCollaborationDialog from "@/components/admin/UpdateCollaborationDialog";
  import DeleteCollaborationDialog from "@/components/admin/DeleteCollaborationDialog";
  ...
  export default function Collaborations() {
    ...
    const {
      ...
      isLoading,
      error,
    } = useCollaborationList({ page, searchQuery: debouncedSearch, sortBy: sort, limit });
  
    const collaborations = collaborationData.result || [];
    const total = collaborationData.total || 0;
  
    const breadcrumbItems = [
      { href: "/dashboard", label: "Dashboard" },
      { label: "Collaborations" },
    ];
    const tableHeaders = ["Brand Name", ""];
    ...
    const handleExport = async () => {
      try {
        setExporting(true);
        await exportData("/collaborations/export", "collaborations");
        showSuccessToast("Collaborations exported successfully");
      } catch (error) {
        showErrorToast("Failed to export collaborations");
        console.error("Failed to export collaborations", error);
      } finally {
        setExporting(false);
      }
    };
      ...
      <>
        ...
        <div className="flex items-center justify-between">
          <div className="flex flex-col items-start gap-y-1.5">
            ...
            <div className="text-sm text-slate-500">
              Manage and view all collaborations
            </div>
          </div>
        ...
        <Card>
          <CardContent className="p-6 md:px-8 md:py-10 space-y-4">
            ...
            <UpdateCollaborationDialog
              open={rowAction?.type === "update"}
              onOpenChange={() => setRowAction(null)}
              collaboration={rowAction?.data ?? null}
            />
